...
func TestCancelIfPaidForRefundCancelsMatchingPaidOrder(t *testing.T) {
	...
	if err := order.MarkPaid("pay-ord-ref-pg-1", order.TotalWon, now); err != nil {
...
func TestCancelIfPaidForRefundSkipsMismatchedPayment(t *testing.T) {
	...
	if err := order.MarkPaid("pay-ord-ref-pg-2", order.TotalWon, now); err != nil {
		t.Fatalf("MarkPaid: %v", err)
	}
	if err := repo.Save(ctx, order); err != nil {
		t.Fatalf("Save: %v", err)
...
func TestCancelIfPaidForRefundIsIdempotent(t *testing.T) {
	...
	if err := order.MarkPaid("pay-ord-ref-pg-3", order.TotalWon, now); err != nil {
		t.Fatalf("MarkPaid: %v", err)
	}
	if err := repo.Save(ctx, order); err != nil {
		t.Fatalf("Save: %v", err)
...
func TestSaveRoundTripConfirmedAndCancelRequest(t *testing.T) {
	...
	if err := order.MarkPaid("pay-policy-1", order.TotalWon, now); err != nil {
		t.Fatalf("MarkPaid: %v", err)
	}

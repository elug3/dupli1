...
func TestRejectCancelRequestHTTP(t *testing.T) {
	pay := &recordingPayment{}
	repo := memory.NewRepository()
	svc := service.New(repo, &fakeStock{}).WithProduct(&fakeProduct{price: 1000}).WithPayment(pay)
	h := handler.New(svc, authjwt.NewHMACValidator(testSecret))
...
func TestApproveCancelRefundRejectedLeavesPaid(t *testing.T) {
	...
	customer := makeToken(t, "u-1", nil)
	w = do(t, mux, http.MethodPost, "/api/v1/orders/"+orderID+"/cancel", customer, map[string]string{"reason": "please"})
	if w.Code != http.StatusOK {
		t.Fatalf("cancel status = %d, want 200; body: %s", w.Code, w.Body.String())
	}
...
func TestGetOrderIncludesRefundPolicyFields(t *testing.T) {
	...
	if raw["immediate_cancel_allowed"] != true {
		t.Fatalf("immediate_cancel_allowed = %v, want true before confirm", raw["immediate_cancel_allowed"])
	}
	if raw["confirmation_due_at"] == nil {
		t.Fatal("confirmation_due_at must be set for unconfirmed paid order")
	}

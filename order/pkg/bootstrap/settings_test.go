...
func TestBuildSettings_PublishesShippingFee(t *testing.T) {
	resp := bootstrap.BuildSettings(bootstrap.Config{ShippingFeeCents: 30000})
	got, ok := resp.Limits["shipping_fee_cents"]
	if !ok {
		t.Fatalf("limits missing shipping_fee_cents: %+v", resp.Limits)
	}
	if got != int64(30000) {
		t.Fatalf("shipping_fee_cents = %v (%T), want int64 30000", got, got)
	}
	...
	if v, ok := free.Limits["shipping_fee_cents"]; !ok || v != int64(0) {

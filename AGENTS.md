...
- **Payment flow:** `POST /api/v1/payments` → manager **Bypass** (`payment.bypass`) in production, or local **dev simulate** when `PAYMENT_ALLOW_DEV_SIMULATE=true`. On success, payment publishes **`payment.succeeded`**; order service marks order **`paid`**. Ship via `POST /api/v1/orders/{id}/ship` → **`in_transit`** (commits stock). See [docs/payment-service.md](docs/payment-service.md).
...
- **SMTP and OAuth providers** are external. There is **no Stripe/card PG adapter** — v1.0 production uses manager **Bypass** only. Local dev simulate requires **`PAYMENT_ALLOW_DEV_SIMULATE=true`** (Compose default); production must leave that env unset.

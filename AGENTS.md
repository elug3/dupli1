...
- **Payment flow:** `POST /api/v1/payments` → Stripe Checkout (when configured), manager **Bypass** (`payment.bypass`), or local dev simulate URL. On success, payment publishes **`payment.succeeded`**; order service marks order **`paid`**. Ship via `POST /api/v1/orders/{id}/ship` → **`in_transit`** (commits stock). See [docs/payment-service.md](docs/payment-service.md).
...
- **SMTP and OAuth providers** are external. **Stripe** is optional — v1.0 production uses manager **Bypass** only (no PG). Local dev simulate requires **`PAYMENT_ALLOW_DEV_SIMULATE=true`** (Compose default) **and** no `STRIPE_SECRET_KEY`; production must leave that env unset.

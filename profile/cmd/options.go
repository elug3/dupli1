...
type Options = profile.ServerOptions

func ConfigureOptions(fs *flag.FlagSet, args []string) (Options, error) {
	opts := profile.NewServerOptions()
	applyEnv(opts)
...
func applyEnv(opts *profile.ServerOptions) {
	if v := os.Getenv("DUPLI1_PROFILE_ADDR"); v != "" {
		opts.Addr = v
	}
	...
	if v := os.Getenv("DUPLI1_PROFILE_DB"); v != "" {
		opts.DatabaseConnString = v
	} else if v := os.Getenv("DB_URL"); v != "" {
		opts.DatabaseConnString = v
	}
	if v := os.Getenv("DUPLI1_PROFILE_NATS_URL"); v != "" {
		opts.NATSURL = v

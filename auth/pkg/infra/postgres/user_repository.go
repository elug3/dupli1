...
func (r *UserRepository) FindByEmail(ctx context.Context, email string) (*domain.User, error) {
	query := `SELECT id, email, password, roles, is_active, locked_at, failed_login_attempts
...
func (r *UserRepository) Save(ctx context.Context, user *domain.User) error {
	query := `INSERT INTO users (id, email, password, roles, is_active, locked_at, failed_login_attempts)
	          VALUES ($1, $2, $3, $4, $5, $6, $7)
	          ON CONFLICT (id) DO UPDATE
	            SET email = $2, password = $3, roles = $4,
	                is_active = $5, locked_at = $6, failed_login_attempts = $7`
	_, err := r.db.ExecContext(ctx, query,
		user.ID, user.Email, user.Password, pq.Array(user.Roles),
...
func scanUser(s scanner) (*domain.User, error) {
	...
	err := s.Scan(
		&u.ID, &u.Email, &u.Password, pq.Array(&u.Roles),

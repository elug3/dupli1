...
func (h *Handler) CreateSubcategory(w http.ResponseWriter, r *http.Request) {
	...
	created, err := h.catalogSvc.CreateSubcategory(domain.Subcategory{Code: body.Code, Name: body.Name})
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) UpdateSubcategory(w http.ResponseWriter, r *http.Request) {
	...
	updated, err := h.catalogSvc.UpdateSubcategoryName(r.PathValue("code"), body.Name)
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) DeleteSubcategory(w http.ResponseWriter, r *http.Request) {
	if err := h.catalogSvc.DeleteSubcategory(r.PathValue("code")); err != nil {
		h.respondServiceError(w, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
// --- Occasions (product look/style; distinct from brand SKU styles) ---

func (h *Handler) ListOccasions(w http.ResponseWriter, r *http.Request) {
	list, err := h.catalogSvc.ListOccasions()
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) CreateOccasion(w http.ResponseWriter, r *http.Request) {
	var body codeNameBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		h.respondError(w, http.StatusBadRequest, "invalid json")
		return
	}
	created, err := h.catalogSvc.CreateOccasion(domain.Occasion{Code: body.Code, Name: body.Name})
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) UpdateOccasion(w http.ResponseWriter, r *http.Request) {
	var body nameBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		h.respondError(w, http.StatusBadRequest, "invalid json")
		return
	}
	updated, err := h.catalogSvc.UpdateOccasionName(r.PathValue("code"), body.Name)
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) DeleteOccasion(w http.ResponseWriter, r *http.Request) {
	if err := h.catalogSvc.DeleteOccasion(r.PathValue("code")); err != nil {
		h.respondServiceError(w, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func (h *Handler) CreateTarget(w http.ResponseWriter, r *http.Request) {
	...
	created, err := h.catalogSvc.CreateTarget(domain.Target{Code: body.Code, Name: body.Name})
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) UpdateTarget(w http.ResponseWriter, r *http.Request) {
	...
	updated, err := h.catalogSvc.UpdateTargetName(r.PathValue("code"), body.Name)
	if err != nil {
		h.respondServiceError(w, err)
		return
	}
...
func (h *Handler) DeleteTarget(w http.ResponseWriter, r *http.Request) {
	if err := h.catalogSvc.DeleteTarget(r.PathValue("code")); err != nil {
		h.respondServiceError(w, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)

...
`POST /orders` and checkout item APIs accept client `unit_price_won`. Totals become NANO / Bypass charge amounts. Cart already ignores client prices.
...
3. In `CreateOrder`, `SetCheckoutItems`, `UpsertCheckoutItem`: resolve each line from product; set `UnitPriceWon = money.FromProductPrice(variant.Price)`; reject missing/inactive variants; **ignore** any client price.
4. Handler request DTO becomes `{sku|sku_id, quantity}` only (like cart `ItemInput`). Keep `unit_price_won` on **responses** and DB as a snapshot.
...
7. Test: client sends `unit_price_won: 1` → stored/charged amount is catalog price.

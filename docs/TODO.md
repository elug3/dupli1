...
- [ ] **Promotional codes — sign-up campaign** (marketing-driven; **on the critical path**, no longer deferred behind v1.0/v1.1). Plan: [product-promo-referral-code-plan.md](product-promo-referral-code-plan.md). Campaign: single-user code **auto-issued on `user.registered`**, **fixed ₩ off**, **minimum spend**, real expiry.
  ...
  - [ ] **Two exit criteria are claimed but untested.** Both behaviours are implemented and believed correct; neither has a test, so a refactor would not be caught:
    - [ ] An `in_transit` cancel must **keep** the redemption consumed. Only the pre-shipment release is tested (`TestCancelBeforeShipmentReleasesThePromotion`). The guard is `if !stockCommitted` in `order/pkg/service/service.go`; the expensive direction to break is releasing after shipment — goods gone *and* the discount handed back
    - [ ] A customer must not use another customer's entitlement. Covered only implicitly by "an account with no entitlement is refused"; there is no test that customer B is refused a code customer A holds
  - [ ] **Dry run the campaign on the dev stack** before enabling it. NATS, the `user.registered` subscriber and the seed have never run together in one process — every test so far calls the issuer's `Handle` directly. Register an account, confirm the entitlement lands in the wallet, then walk one order through checkout → paid → cancel and watch the use come back
  - [ ] **`nginx -t` was never run** against the `/api/v1/promotions` locations added in Phase 1. Image pulls were blocked in the authoring environment, so the three configs were only checked structurally (brace balance, and each new block is a copy of the adjacent working one). Verify with `sudo docker compose up -d --build dupli1-proxy` somewhere with registry access
  - [ ] **Revoke has no UI.** `revokePromotionEntitlement` and the backend `DELETE …/promotions/entitlements/{id}` both exist and are unused. It needs a per-customer entitlement list to revoke *from*, and the only list endpoint today is `…/promotions/me`, which is ABAC self-only — so a manager cannot see what a customer holds. Needs a `promotion.issue`-gated list endpoint first, then the UI beside the issue form on user detail
  - [ ] **Storefront wallet has no automated coverage.** The wallet client, the checkout picker and the profile list are untested; `dupli1-web`'s suite covers `lib/` only. `npm run test:orders:browser` is the existing pattern for driving a real browser against the mock gateway
...
- [ ] **API path convention** — `/api/v1/{service_name}/...` only. Canonical paths added; legacy top-level aliases kept until clients migrate, then remove.
  ...
  - Mapping:
    ...
    | `/api/v1/coupons` | `/api/v1/products/promotions` ([renamed](product-promotion-rename.md); `/api/v1/products/coupons` also still answers) |

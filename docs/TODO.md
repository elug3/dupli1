...
- [x] **Parent + variants** — implemented; see [product-variants-plan.md](product-variants-plan.md). Remaining: drop legacy parent `color`/`stock`/`imageUrls` columns, merge pre-existing duplicate color products.
- [ ] **Product stock tracking** — always-tracked SKUs, PDP `inStock`/`availableQty`, cart stock-on-add; plan in [product-stock-tracking-plan.md](product-stock-tracking-plan.md).

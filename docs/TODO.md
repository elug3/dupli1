...
- [x] **SKU master-data runtime CRUD** — Phase A+B: styles table, FKs, catalog APIs (`/api/v1/catalog/...`), `product.master.read|write`; see [product-sku-master-data-plan.md](product-sku-master-data-plan.md). Phase C (strict enforce on SKU writes) + D (admin UI) remain.

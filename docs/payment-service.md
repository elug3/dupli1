...
Order consumer: idempotent on `payment_id`; reject if `amount_cents != order.total_cents`. If the order already carries the same `payment_id` and is no longer `pending` (e.g. already `in_transit` after ship), `MarkOrderPaid` is a no-op — the payment reconcile worker republishes `payment.succeeded` for up to two hours as a guard against lost Core NATS deliveries.
...
| Replayed `payment.succeeded` after ship/fulfill | no-op when `payment_id` already matches — status unchanged |

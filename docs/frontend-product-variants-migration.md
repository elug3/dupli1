...
- `sku` = **variant.sku**
- `unit_price_won` = `Math.round(product.price)` (or `variant.price`, which mirrors the parent) — product prices are already **KRW won**; do **not** multiply by 100 (KRW is a zero-decimal Stripe currency). JSON field name remains `*_won` for historical reasons; the value is whole won.

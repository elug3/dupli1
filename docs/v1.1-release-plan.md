...
Assumption: v1.0 exit criteria in [v1-release-plan.md](v1-release-plan.md) are met before v1.1 is tagged.
...
| # | Goal | Outcome |
|---|------|---------|
| 1 | **Logging** | Stable machine `code` on API errors; `slog` / structured fields on all services |
...
**Outcome:** Predictable error JSON + structured server logs across all services.
...
| Order / cart / payment | `{ "error": "…", "code": <http status int> }` |
| Auth (Gin) | `{ "error": "…" }` only; already uses zerolog for some events |
| Other services | Ad hoc `log.Printf` on many paths |
...
**Target log contract:** `service`, `op`, `code`, `error`, and when available `request_id`, `route`, `user_id` / `customer_id` (no secrets, no refresh tokens in logs).

| Step | Work |
|------|------|
| 1 | Shared HTTP error helper in `shared/` (stdlib + Gin adapter) |
| 2 | Code catalog: auth sessions, cart, checkout, order, payment, inventory |
| 3 | Migrate handlers; align auth zerolog with shared field names |
| 4 | Update [api.md](api.md), OpenAPI, tests |

**Done when:** Logs and responses are grep-friendly by `code`; no SQL/driver text in JSON bodies.
...
- [ ] Stable machine `code` on API errors; structured logs on failure paths

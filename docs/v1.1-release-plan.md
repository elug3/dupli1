...
**Status:** Planning (2026-07-26) — **blocked on v1.0**, whose **2026-08-30 target was missed** (postponed 2026-07-27; ships on [v1.0-release-spec.md](v1.0-release-spec.md) closeout, no new date set).  
**Not a blocker for promotional codes:** that work was resequenced onto the critical path on 2026-09-16 and runs in parallel with v1.0 closeout — [product-promo-referral-code-plan.md](product-promo-referral-code-plan.md).  
**Depends on:** [v1-release-plan.md](v1-release-plan.md) (v1.0 launch cut + [schedule](v1-release-plan.md#schedule))  
...
**v1.0** ships a reliable KRW checkout loop (catalog → cart → order → NANO/Bypass → ship → stock commit).
...
| Manager settings API, Redis cache | Product surface — v1.2 |
...
| Theme | Items |
|-------|--------|
| Commerce UX | Guest cart + merge; cart checkout helper; stock on add + PDP `inStock` **done** — [product-stock-tracking-plan.md](product-stock-tracking-plan.md); promo/referral codes — [product-promo-referral-code-plan.md](product-promo-referral-code-plan.md) |
| Payments | Paid cancel → PG refund (NANO / ops) |
| Discovery | Co-view recommendations |
| API cleanup | Legacy path alias removal |
| Platform product | Manager settings; Redis cache; SKU master Phase D UI |

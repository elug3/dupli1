...
**Related:** [TODO.md](TODO.md), [current-state.md](current-state.md), [cart-service.md](cart-service.md), [payment-service.md](payment-service.md), [payment-methods-plan.md](payment-methods-plan.md), [product-guest-views-plan.md](product-guest-views-plan.md), [product-views-recommendations-plan.md](product-views-recommendations-plan.md), [manager-settings-api.md](manager-settings-api.md), [quality-bugs-fix-plan.md](quality-bugs-fix-plan.md).
...
**v1.1** makes that loop usable for anonymous shoppers, safer for ops (refunds), cleaner for clients (drop legacy API aliases), and slightly smarter on the PDP (co-view recs + stock hints). It is still **not** “platform complete”: Bitcoin, chat, analytics services, and email/SMS stay later.
...
### Slice 3 — Legacy alias removal (P1)
...
### Slice 4 — Co-view recommendations (P1)
...
### Slice 5 — Correctness & latency (P2)
...
### Slice 6 — Manager settings + cart convenience (P2)
...
flowchart LR
  ...
  S2[Slice 2 Refunds]
  S3[Slice 3 Alias removal]
  S4[Slice 4 Co-view]
  S5[Slice 5 H6 + cache]
  S6[Slice 6 Settings + cart UX]
  S7[Slice 7 Platform]
...
- **S1 and S2 in parallel** (different services).
- **S3** after frontend migration signal.
- **S4** independent of cart/payment.
- **S5** anytime after P0; nice before heavy traffic.
- **S6** after S1 if checkout helper should support guest→login flow; settings can start earlier.
- **S7** continuous / opportunistic.

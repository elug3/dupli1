...
Assumption: v1.0 exit criteria in [v1.0-release-spec.md](v1.0-release-spec.md) are met before v1.1 is tagged.
...
| Piece | State |
|-------|--------|
| Auth | **zerolog** — JSON or console via `LogOutput` / `LogLevel`; handlers use `.Str("event", …).Err(err).Msg(…)` |

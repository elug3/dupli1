...
| Promotional codes | **Partial** — flat percent-off CRUD + redeem + order apply work; expiry is **not enforced**, there is **no usage accounting**, and there is no wallet. Renamed `coupon` → `promotion` ([product-promotion-rename.md](product-promotion-rename.md), done); the rest is Phases 2–3 of [product-promo-referral-code-plan.md](product-promo-referral-code-plan.md) |

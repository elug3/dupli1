...
| Promotional codes (today: `coupons`) | **Partial** — flat percent-off CRUD + redeem + order apply work; expiry is **not enforced**, there is **no usage accounting**, and there is no wallet. Being built out, and renamed `coupon` → `promotion`, per [product-promo-referral-code-plan.md](product-promo-referral-code-plan.md) / [product-promotion-rename.md](product-promotion-rename.md) |
...
- **Features:**
  ...
  - `dupli1-order` service account: `order.ship`, `order.status.update`, `inventory.reservation.manage`, `payment.cancel` (`DUPLI1_ORDER_SERVICE_*`); order refreshes a Bearer access token and calls product stock/coupons **and payment cancel** via **`DUPLI1_GATEWAY_URL`** (`httpstock` / `httppayment` / gateway paths)
...
- **Features:**
  ...
  - **Promotional codes (`coupons` today):** `code`, `discount` (fraction), `description`, `expires` (**free text, never compared — expiry is not enforced**), `active`; PG + in-memory stores, seeds `SUMMER30`. `POST /api/v1/products/coupons/redeem` is a **public, unthrottled lookup** with no per-customer limit, so a code is reusable without bound. `discount` is not validated on write, so a definition outside `(0,1)` saves and then fails at order apply. Being replaced by the promotion model (scope, `expires_at`, conditions/benefit JSONB, redemption ledger, wallet) — [product-promo-referral-code-plan.md](product-promo-referral-code-plan.md)

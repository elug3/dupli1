...
- **Features:**
  ...
  - `dupli1-order` service account: `order.ship`, `order.status.update`, `inventory.reservation.manage` (`DUPLI1_ORDER_SERVICE_*`); order refreshes the inventory Bearer access token (login → refresh, re-login on refresh failure)

...
- **Auth:** Bearer JWT via `AUTH_JWKS_URL` (RS256 JWKS; HS256 fallback in dev). Storefront ABAC on `customer_id`; `order.create` / `order.read.all` bypass ABAC. Ship requires `order.ship`; status changes, confirm, and cancel-request approve/reject require `order.status.update`. Customer cancel is owner ABAC.

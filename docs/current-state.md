...
- **Features:**
  ...
  - **Shipping fee:** flat per-order delivery charge via `DUPLI1_ORDER_SHIPPING_FEE_CENTS` (whole KRW, default 0 = free). `total = subtotal - discount + shipping`; no free-shipping threshold; coupons discount goods only. Snapshotted on the order and on the checkout session so a config change never re-prices existing ones

...
| # | Finding | Status |
|---|---------|--------|
| C1 | **Client-controlled order prices** — `POST /orders` and checkout item APIs accept `unit_price_won` from the client; totals and payment amounts derive from that. | **Fixed** — order/checkout resolve prices from product (client `unit_price_won` ignored) |
...
| Payment | publish-after-save retry (now covered); NANO return/webhook |
...
1. ~~**Server-side pricing** at order/checkout create (ignore client `unit_price_won`)~~ **done** — see [quality-bugs-fix-plan.md](quality-bugs-fix-plan.md)

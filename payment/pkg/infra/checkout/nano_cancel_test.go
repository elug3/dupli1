...
func TestCancelPayment_UnconfiguredProvider(t *testing.T) {
	...
	_, err := p.CancelPayment(t.Context(), ports.CancelPaymentInput{
		ProviderRef: "2409030071109", PaymentID: "pay_000001", AmountWon: 70000,
	})
	if !errors.Is(err, ports.ErrCancelUnsupported) {
		t.Fatalf("err = %v, want ErrCancelUnsupported", err)
	}
...
func TestCancelPayment_Success(t *testing.T) {
	...
	if res.CanceledAmountWon != 70000 {
		t.Fatalf("canceled = %d, want 70000", res.CanceledAmountWon)
	}
	if !res.RemainingKnown || res.RemainingWon != 0 {
		t.Fatalf("remaining = %d (known=%t), want 0 known", res.RemainingWon, res.RemainingKnown)
	}
...
func TestCancelPayment_PartialReportsRemaining(t *testing.T) {
	...
	p := cancelTestProvider(srv.URL)
	res, err := p.CancelPayment(t.Context(), ports.CancelPaymentInput{
		ProviderRef: "2409030071109", PaymentID: "pay_000001", AmountWon: 20000,
	})
	if err != nil {
		t.Fatalf("CancelPayment: %v", err)
	}
	if res.CanceledAmountWon != 20000 {
		t.Fatalf("canceled = %d, want 20000", res.CanceledAmountWon)
	}
	if !res.RemainingKnown || res.RemainingWon != 50000 {
		t.Fatalf("remaining = %d (known=%t), want 50000 known", res.RemainingWon, res.RemainingKnown)
	}
}
...
func TestCancelPayment_MissingRemainAmtIsUnknownNotZero(t *testing.T) {
	...
	p := cancelTestProvider(srv.URL)
	res, err := p.CancelPayment(t.Context(), ports.CancelPaymentInput{
		ProviderRef: "2409040000019", PaymentID: "pay_000001", AmountWon: 1004,
	})
	if err != nil {
		t.Fatalf("CancelPayment: %v", err)
	}
	...
	if res.CanceledAmountWon != 1004 {
		t.Fatalf("canceled = %d, want 1004", res.CanceledAmountWon)
	}
}
...
func TestUnavailableProviderCancelUnsupported(t *testing.T) {
	...
	_, err := p.CancelPayment(t.Context(), ports.CancelPaymentInput{AmountWon: 100})
	if !errors.Is(err, ports.ErrCancelUnsupported) {
		t.Fatalf("err = %v, want ErrCancelUnsupported", err)
	}
}
...
func TestPlaceholderProviderRefRoundTrip(t *testing.T) {
	...
	sess, err := p.CreateSession(t.Context(), ports.CheckoutSessionInput{
		OrderID: "ord_1", PaymentID: "pay_000001", AmountWon: 70000,

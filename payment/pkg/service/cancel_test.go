...
func (p *cancelProvider) CancelPayment(_ context.Context, input ports.CancelPaymentInput) (*ports.CancelPaymentResult, error) {
	...
	return &ports.CancelPaymentResult{CanceledAmountWon: input.AmountWon}, nil
}
...
func seedSucceededCardPayment(t *testing.T, repo ports.Repository, provider ports.CheckoutProvider, pub ports.EventPublisher, totalKRW int64) (*service.Service, *domain.Payment) {
	...
	orders := stubOrderClient{order: &ports.OrderSummary{
		ID: "ord_1", CustomerID: "cust_1", Status: "pending", TotalWon: totalKRW,
...
func TestCancelPayment_FullCancelCallsProviderAndClosesPayment(t *testing.T) {
	...
	if call.AmountWon != 70000 {
		t.Fatalf("provider asked to cancel %d, want the full 70000", call.AmountWon)
	}
	...
	if stored.Status != domain.StatusCanceled || stored.CanceledAmountWon != 70000 {
		t.Fatalf("stored = %q / %d", stored.Status, stored.CanceledAmountWon)
	}
	...
	if ev.AmountWon != 70000 || ev.RemainingWon != 0 {
		t.Fatalf("event amounts = %d / %d", ev.AmountWon, ev.RemainingWon)
	}
}
...
func TestCancelPayment_PartialLeavesRemainder(t *testing.T) {
	...
	provider := &cancelProvider{
		result: &ports.CancelPaymentResult{
			CanceledAmountWon: 20000, RemainingWon: 50000, RemainingKnown: true,
		},
	}
	pub := &cancelEventPublisher{}
	svc, seeded := seedSucceededCardPayment(t, repo, provider, pub, 70000)
	...
	canceled, err := svc.CancelPayment(t.Context(), service.CancelPaymentInput{
		PaymentID: seeded.ID, AmountWon: 20000,
	})
	if err != nil {
		t.Fatalf("CancelPayment: %v", err)
	}
	...
	if len(pub.events) != 1 || pub.events[0].RemainingWon != 50000 {
...
func TestCancelPayment_ProviderRejectionLeavesPaymentUntouched(t *testing.T) {
	...
	if stored.CanceledAmountWon != 0 || stored.CanceledAt != nil {
		t.Fatalf("rejected cancel left state: %d / %v", stored.CanceledAmountWon, stored.CanceledAt)
	}
	if len(pub.events) != 0 {
...
func TestCancelPayment_IdempotencyKeyBlocksDoublePartialRefund(t *testing.T) {
	...
	first, err := svc.CancelPayment(t.Context(), service.CancelPaymentInput{
		PaymentID: seeded.ID, AmountWon: 20000, IdempotencyKey: "retry-1",
	})
	if err != nil {
	...
	if second.CanceledAmountWon != first.CanceledAmountWon {
		t.Fatalf("retry changed total: %d then %d", first.CanceledAmountWon, second.CanceledAmountWon)
	}
	if second.CanceledAmountWon != 20000 {
		t.Fatalf("canceled = %d, want 20000", second.CanceledAmountWon)
	}
}
...
func TestCancelPayment_DifferentKeyRefundsAgain(t *testing.T) {
	...
	if _, err := svc.CancelPayment(t.Context(), service.CancelPaymentInput{
		PaymentID: seeded.ID, AmountWon: 20000, IdempotencyKey: "cancel-1",
	}); err != nil {
		t.Fatalf("first partial: %v", err)
	}
	got, err := svc.CancelPayment(t.Context(), service.CancelPaymentInput{
		PaymentID: seeded.ID, AmountWon: 30000, IdempotencyKey: "cancel-2",
	})
	if err != nil {
	...
	if got.CanceledAmountWon != 50000 {
		t.Fatalf("canceled = %d, want 50000 cumulative", got.CanceledAmountWon)
	}
...
func TestCancelPayment_RejectsAmountOverRemaining(t *testing.T) {
	...
	_, err := svc.CancelPayment(t.Context(), service.CancelPaymentInput{
		PaymentID: seeded.ID, AmountWon: 70001,
	})
...
func TestCancelPayment_ProviderRemainingWins(t *testing.T) {
	...
	provider := &cancelProvider{
		result: &ports.CancelPaymentResult{
			CanceledAmountWon: 20000, RemainingWon: 40000, RemainingKnown: true,
		},
	}
	svc, seeded := seedSucceededCardPayment(t, repo, provider, &cancelEventPublisher{}, 70000)
	...
	if got.CanceledAmountWon != 30000 {
		t.Fatalf("canceled = %d, want 30000 derived from the provider's remainAmt", got.CanceledAmountWon)
	}
...
func TestCancelPayment_ProviderOvershootIsClampedNotDropped(t *testing.T) {
	repo := memory.NewRepository()
	provider := &cancelProvider{
		result: &ports.CancelPaymentResult{CanceledAmountWon: 999999},
	}
	svc, seeded := seedSucceededCardPayment(t, repo, provider, &cancelEventPublisher{}, 70000)
	...
	if got.CanceledAmountWon != 70000 {
		t.Fatalf("canceled = %d, want it clamped to the 70000 total", got.CanceledAmountWon)
	}
}
...
func TestCancelPayment_ConcurrentCancelsCallProviderOnce(t *testing.T) {
	...
	if stored.Status != domain.StatusCanceled || stored.CanceledAmountWon != 70000 {
		t.Fatalf("stored = %q / %d, want a single full cancel", stored.Status, stored.CanceledAmountWon)
	}
}

...
- **Promotional codes, not coupons.** The product term is **"promotional code"** (프로모션 코드). The code still says `coupon` everywhere — table `coupons`, `coupon_code`, `coupon.*` permissions, `/api/v1/products/coupons` — and a full rename to `promotion` is Phase 1 of the plan, not yet done. Write new prose and UI copy as "promotional code"; do not rename identifiers ad hoc, follow [docs/product-promotion-rename.md](docs/product-promotion-rename.md). `discount_won` keeps its name.
